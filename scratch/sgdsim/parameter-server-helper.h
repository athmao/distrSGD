...
public:
  ...
  /**
   * Create a ParameterServerApplication on the specified Node.
   *
  ...
  /**
   * Create a ParameterServerApplication on specified node
   *
...
private:
  /**
   * Install an ns3::ParameterServer on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an ParameterServer will be installed.
 ...
 */
class ParameterClientHelper
{
public:
  /**
   * Create ParameterClientHelper which will make life easier for people trying
   ...
   */
  ParameterClientHelper (Address ip, uint16_t port);
  /**
   * Create ParameterClientHelper which will make life easier for people trying
   * to set up simulations with echos. Use this variant with addresses that do
   ...
   */
  ParameterClientHelper (Address addr);
   ...
   *
   * \param node The Ptr<Node> on which to create the ParameterClientApplication.
   *
   ...
   *
   * \param nodeName The name of the node on which to create the ParameterClientApplication
   *
   * \returns An ApplicationContainer that holds a Ptr<Application> to the
   *          application created
   */
...
private:
  /**
   * Install an ns3::ParameterClient on the node configured with all the
   * attributes set with SetAttribute.
   *
   * \param node The node on which an ParameterClient will be installed.
   * \returns Ptr to the application installed.
   */
  Ptr<Application> InstallPriv (Ptr<Node> node) const;
  ObjectFactory m_factory; //!< Object factory.
